...
function RiskBadge({ level }: { level: "Low" | "Medium" | "High" }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${map[level]}`}>
      {level} risk
    </span>
  );
}
...
export default function SkinSightUploadAnalyze({
  ...
  const onFile = (f?: File | null) => {
    ...
    console.log("type:", f.type || "(unknown)");
  ...
  // -------- Analyze flow (with logging) --------
  const analyze = useCallback(async () => {
    if (!file) return;

    // Start logging group + timer
    ...
    try {
      ...
      if (fetchResources && res?.result?.condition) {
        ...
        try {
          ...
          console.warn("Resource fetch failed (continuing):", e);
        }
        console.groupEnd();
      }
  ...
  const riskClass = useMemo(() => {
    const lvl = data?.result?.risk;
    if (lvl === "High") return "bg-red-50";
    if (lvl === "Medium") return "bg-amber-50";
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_8px_30px_rgba(2,132,199,0.08)]">
        <div className="p-6">
          ...
          >
            ...
            <button
              onClick={choose}
              className="mt-3 inline-flex items-center px-4 py-2 rounded-lg bg-sky-500 hover:bg-sky-600 text-white font-semibold"
            >
              Browse files
            </button>
          ...
          {file && (
            <div className="mt-4 grid gap-3">
              <div className="text-sm">
                Selected: <span className="text-slate-600">{file.name}</span>
              </div>
              {preview && (
                <img
                  src={preview}
          ...
          <div className="mt-5 flex gap-3">
            <button
              onClick={analyze}
              disabled={!file || busy}
              className="inline-flex items-center px-4 py-2 rounded-lg bg-black text-white font-semibold disabled:opacity-50"
            >
              {busy ? "Analyzing…" : "Analyze photo"}
            </button>
            <button
              onClick={() => {
                console.log("[SkinSight] Reset UI state");
                setFile(null);
                setPreview(null);
                setData(null);
                setError(null);
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_8px_30px_rgba(2,132,199,0.08)]">
          <div className="p-6">
            <div className="flex items-center justify-between gap-2">
              ...
              {data?.result?.risk && <RiskBadge level={data.result.risk} />}
            </div>

            {!data && (
              <p className="text-slate-500 text-sm mt-1">
                Upload an image and click Analyze to see the findings.
              </p>
            )}
            ...
            {data && (
              <div className={`mt-3 rounded-xl p-4 ${riskClass}`}>
                <div className="text-sm text-slate-600">
                  Confidence: {prettyPercent(data.result.confidence)}
                </div>
                <p className="mt-2">{data.result.explanation}</p>

                {!!data.result.possible_causes?.length && (
                  <p className="mt-2 text-sm">
                    <b>Possible causes:</b> {data.result.possible_causes.join(", ")}
                  </p>
                )}
                ...
                {!!data.result.recommended_next_steps?.length && (
                  <div className="mt-3">
                    ...
                    <ul className="list-disc ml-5 text-sm mt-1">
                      {data.result.recommended_next_steps.map((s) => (
                        <li key={s}>{s}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {data.result.when_to_see_doctor && (
                  <p className="mt-2 italic text-sm">{data.result.when_to_see_doctor}</p>
                )}
